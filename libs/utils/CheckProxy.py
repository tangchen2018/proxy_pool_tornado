...
class CheckProxy(object):
    ...
    def checkAllGetProxyFunc():
        ...
        member_list = inspect.getmembers(
            GetFreeProxy, predicate=inspect.isfunction)
        ...
        for func_name, func in member_list:
            log.info(u"函数 {n}, 获取到代理数: {c}".format(
                n=func_name, c=proxy_count_dict.get(func_name, 0)))
    ...
    def checkGetProxyFunc(func):
        ...
        for proxy in func():
            if verifyProxyFormat(proxy):
                ...
                count += 1
        log.info("{n} completed, fetch proxy number: {c}".format(
            n=func_name, c=count))
